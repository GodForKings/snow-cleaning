...
const inputVariants = cva(
	`bg-background border-input text-foreground placeholder:text-muted-foreground/80 focus-visible:ring-ring/30 focus-visible:border-ring [&[readonly]]:bg-muted/80 file:border-input file:text-foreground aria-invalid:border-destructive/60 aria-invalid:ring-destructive/10 dark:aria-invalid:border-destructive dark:aria-invalid:ring-destructive/20 flex w-full border shadow-xs shadow-black/5 transition-[color,box-shadow] file:h-full file:border-0 file:border-e file:border-solid file:bg-transparent file:p-0 file:font-medium file:not-italic focus-visible:ring-[3px] focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-60 [&[readonly]]:cursor-not-allowed [&[type=file]]:py-0`,
	{
		variants: {
			variant: {
...
const inputWrapperVariants = cva(
	`has-[:focus-visible]:ring-ring/30 has-[:focus-visible]:border-ring [&_[data-slot=input]]:text-foreground [&_[data-slot=input]]:placeholder:text-muted-foreground [&_svg]:text-muted-foreground has-[[aria-invalid=true]]:border-destructive/60 has-[[aria-invalid=true]]:ring-destructive/10 dark:has-[[aria-invalid=true]]:border-destructive dark:has-[[aria-invalid=true]]:ring-destructive/20 flex items-center gap-1.5 has-[:focus-visible]:ring-[3px] has-[:focus-visible]:outline-none [&_[data-slot=datefield]]:grow [&_[data-slot=input]]:flex [&_[data-slot=input]]:h-auto [&_[data-slot=input]]:w-full [&_[data-slot=input]]:border-0 [&_[data-slot=input]]:bg-transparent [&_[data-slot=input]]:p-0 [&_[data-slot=input]]:shadow-none [&_[data-slot=input]]:transition-colors [&_[data-slot=input]]:outline-none [&_[data-slot=input]]:focus-visible:ring-0 [&_[data-slot=input]]:disabled:cursor-not-allowed [&_[data-slot=input]]:disabled:opacity-50 [&_[data-slot=input]]:data-focus-within:border-0 [&_[data-slot=input]]:data-focus-within:ring-0 [&_[data-slot=input]]:data-focus-within:ring-transparent [&_svg]:shrink-0`,
	{
		variants: {
			variant: {
