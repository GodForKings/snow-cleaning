...
const accordionItemVariants = cva('', {
	variants: {
		variant: {
			...
			outline: 'border-border rounded-lg border px-4',
			solid: 'bg-accent/70 rounded-lg px-4',
		},
	},
	defaultVariants: {
		variant: 'default',
...
const accordionTriggerVariants = cva(
	'text-foreground flex flex-1 cursor-pointer items-center justify-between gap-2.5 py-4 font-medium transition-all [&[data-panel-open]>svg]:rotate-180',
	{
		variants: {
			variant: {
				default: '',
			...
			indicator: {
				arrow: '',
				plus: '[&>svg>path:last-child]:origin-center [&>svg>path:last-child]:transition-all [&>svg>path:last-child]:duration-200 [&[data-panel-open]>svg]:rotate-180 [&[data-panel-open]>svg>path:last-child]:rotate-90 [&[data-panel-open]>svg>path:last-child]:opacity-0',
				none: '',
			},
		},
		defaultVariants: {
...
const accordionPanelVariants = cva(
	'text-accent-foreground h-[var(--accordion-panel-height)] overflow-hidden text-sm transition-[height] ease-out data-[ending-style]:h-0 data-[starting-style]:h-0',
	{
		variants: {
			variant: {
				default: '',
